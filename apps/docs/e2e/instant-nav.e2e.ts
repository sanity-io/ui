...
test('the arcade keeps its fill-height layout', async ({page}) => {
  test.skip(
    page.viewportSize()!.width < SIDEBAR_BREAKPOINT,
    'the arcade switches to a stacked layout below media[1]',
  )
  await page.goto('/ui/arcade')
  const arcade = page.getByTestId('arcade-screen')
  await expect(arcade).toBeVisible({timeout: 30_000})

  const {height, viewportHeight} = await arcade.evaluate((node) => ({
    height: node.getBoundingClientRect().height,
    viewportHeight: window.innerHeight,
  }))
  expect(height).toBeGreaterThan(viewportHeight * 0.7)
})
