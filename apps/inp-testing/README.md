...
3. Load one library's page. Click each section's control 5 times, noting "Last interaction" each time. Section 4 alternates mount/unmount — note both.
...
Test sizes (`ROW_COUNT`, `TONE_CARD_COUNT`, `PANEL_CARD_COUNT`) are constants at the top of each route file. Keep both files in sync when changing them.
