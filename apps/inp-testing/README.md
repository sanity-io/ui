...
Each page has five test sections. Every control is wired to React state, so a click forces the library to re-render real UI. The sections differ in how much UI one click redraws (the "blast radius"):
...
3. Load one library's page. Click each section's control 5 times, noting "Last interaction" each time. Sections 4 and 5 alternate mount/unmount — note both.
...
Test sizes (`ROW_COUNT`, `TONE_CARD_COUNT`, `PANEL_CARD_COUNT`, `TOOLTIP_COUNT`) are constants at the top of each route file. Keep both files in sync when changing them.
