...
- `pnpm knip` runs [knip](https://knip.dev) (config in `knip.jsonc`, also a CI
  job) to detect unused files, dependencies and exports. Exports that are only
  used within the same module should not be exported (knip reports them; only
  types/interfaces are exempt via `ignoreExportsUsedInFile`). Dependencies
  that are referenced but invisible to knip (e.g. only as strings in babel
  plugin arrays) are listed in `ignoreDependencies` with a comment explaining
  why. `pnpm check:deps` runs only the dependency checks.
