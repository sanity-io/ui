...
- `@sanity/themer` generates Studio themes from a handful of colors with `createTheme({primary, text, lightBackground, darkBackground})` — plus `positive`, `caution` and `critical` for the accent tones — and ships the familiar presets re-expressed for the new API
