...
- Non-obvious: `pnpm build` runs `generate`, which deletes and regenerates `src/exports/*`, `src/icons.ts`, and `src/deprecations.ts` from the SVG sources in `export/`. These generated files are committed; do not hand-edit them—edit the SVGs in `export/` or `scripts/generate.ts` instead.
