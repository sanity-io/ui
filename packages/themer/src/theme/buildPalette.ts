...
/**
 * A color palette generated by {@link buildPalette}: the `palette` config of
 * `buildTheme` from `@sanity/ui/theme`, with every color as a hex string.
 *
 * @public
 */
...
/**
 * The palettes generated by {@link buildPalette} — one per color scheme.
 *
 * @public
 */
export type GeneratedColorPalettes = Record<ThemeColorSchemeKey, GeneratedColorPalette>

/**
 * Generates a Sanity UI color palette for each color scheme from the theme
 * options: in each palette the scheme's accent color replaces the `blue`
 * scale, its text color the `gray` scale, and the backgrounds replace `black`
 * and `white`. All other hues keep their stock `@sanity/color` values. Called
 * with the stock colors, both palettes come back as the stock palette
 * byte-for-byte.
 *
 * ```ts
 * import {buildPalette} from '@sanity/themer'
 *
 * const {light, dark} = buildPalette({light: {accent: '#556bfc'}})
 * ```
 *
 * @public
 */
export function buildPalette(options: BuildThemeOptions = {}): GeneratedColorPalettes {
  const resolved = resolveThemeOptions(options)

  return {
    dark: buildSchemePalette('dark', resolved.dark, resolved.light.background),
    light: buildSchemePalette('light', resolved.light, resolved.dark.background),
  }
}

/**
 * Generates the palette of one scheme. The scheme's own background takes the
 * slot the scheme paints with (`black` for dark, `white` for light), while
 * the other slot — which a scheme only touches for a few inverted details —
 * takes the other scheme's background, so the theme stays one palette.
 */
function buildSchemePalette(
  key: ThemeColorSchemeKey,
  scheme: ResolvedSchemeOptions,
  otherBackground: string,
): GeneratedColorPalette {
  const accentAnchor = clampAnchorLightness(hexToHsl(scheme.accent))
  const textAnchor = clampAnchorLightness(hexToHsl(scheme.text))
  const grayAnchor = applyContrast(textAnchor, accentAnchor, scheme.contrast)
  ...
  const darkBackground = key === 'dark' ? scheme.background : otherBackground
  const lightBackground = key === 'light' ? scheme.background : otherBackground

  return {
    black: darkenBackground(darkBackground, {
      anchorLuminances,
      tints: [grayTints['400'], blueTints['400']],
    }),
    white: lightenBackground(lightBackground, anchorLuminances),
    gray: grayTints,
    blue: blueTints,
