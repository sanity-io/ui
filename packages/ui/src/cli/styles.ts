...
const PRESENT = /@sanity-labs\/ui-poc\/styles\.css/;
...
export function inspectStylesheet(cwd: string, entry: string): StylesheetInfo {
  ...
  const roots = new Set(
    [dirname(entry), ...SOURCE_ROOTS].map((r) => join(cwd, r)),
  );
...
export function wireStylesheet(
  ...
  const next = entry.endsWith(".astro")
    ? injectIntoAstro(content)
    : injectIntoModule(content);
