...
import {colorSpec} from './_core.spec'
    ...
    return [
      ...
      _fromEntries(
        CARD_TONES.map((tone) => {
          ...
          const tokenOptions = {defaultHue: toneHue}
            ...
            tone,
            {
              avatar: _fromEntries(
                AVATAR_COLORS.map((color) => {
                  const avatarSpec = _merge(toneSpec.avatar?.[color], toneSpec.avatar?.['*'])
                  ...
                  return [
                    color,
                    {
                      bg: _parseColorExprLiteral(
                        avatarSpec.bg?.[schemeIndex] ?? '500',
                        avatarOptions,
                      ),
                      fg: _parseColorExprLiteral(
                        avatarSpec.fg?.[schemeIndex] ?? 'white',
                        avatarOptions,
                      ),
                    },
                  ]
                ...
                token: _fromEntries(
                  CODE_TOKEN_KEYS.map((key) => [
                    ...
                    _parseColorExprLiteral(
                      toneSpec.code?.token?.[key]?.[schemeIndex] ?? '600',
                      tokenOptions,
                    ),
                  ]),
                ),
