...
- Avoid creating custom titles for stories - these should be inferred via folder structure alone.
  - The only exception to this rule are docs / pages that are ['unattached'](https://storybook.js.org/docs/react/api/doc-block-meta#attached-vs-unattached) such as this page or pages adjacent to `@sanity/ui` (such as the one outlining `@sanity/color` palettes).
