import {Box, Card, Code, Flex, Grid, Heading, Stack, StyleTags} from '@sanity/ui'
...
function ColorTintPreview(props: {hue: (typeof HUES)[number]; tint: (typeof TINTS)[number]}) {
  ...
  return (
    <Card
      ...
      radius={2}
      style={{
        // @ts-expect-error React types don't support CSS variables in style props
        '--color-bg': vars[hue][tint],
        '--color-fg': tint < 500 ? vars.black : vars.white,
      }}
    >
      <Flex padding={3}>
        <Box flex={1}>
